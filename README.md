...
## Change Log
- 2026-09-02: **Receipt preview from album folder:** Persist `file://…/Pictures/Mountain Money/MountainMoney_*.jpg` after import; preview finds that JPEG by filename (folder path, listing, or MediaStore `DISPLAY_NAME`). Reverted `ReceiptAlbumUris` fragment/id lookup. Tests: `ReceiptFolderOpenerTest`. Local Gradle may still need JDK **8–15**.
- 2026-09-02: **Receipt URI after album move:** After a gallery/camera file lands in Pictures/Mountain Money, persist the album MediaStore URI looked up by `MountainMoney_*.jpg` (filename kept in the URI fragment), not the picker URI. Preview re-resolves that folder file if the MediaStore id changed. Tests: `ReceiptAlbumUrisTest`.
- 2026-08-31: **Receipt preview open:** Gallery import always persists the Mountain Money album URI (no rollback to an expired picker grant). Preview/list photo open that stored URI with read grants and do not re-import/move. Decode uses `ReceiptUriStreams` (ContentResolver + `file://` fallback) and byte-array sampling. Android 13+ requests `READ_MEDIA_IMAGES`. Web preview fetches `/api/receipts/:id` with credentials. Tests: `ReceiptPickerUriNormalizerTest.persistUriAfterImport_keepsAlbumCopyWhenPickerSourceRemains`.
