...
- 2026-06-17: **Pond/payday/receipt UX batch:** Pond header (**selected count**, **Select all** / **Unselect all**, drag **Reorder** with persisted JSON order); payday **Still to deposit** = schedule gap + **Payday progress** on rows; split slider **100%** absorbs odd cents; receipt capture **EXIF upright** save; OCR short brand + `$` total scoring. Tests: **`PondBankReconciliationHelperTest`**, **`TransferBucketUiHelperTest`**, **`ReceiptFieldParserTest`**, **`ReceiptExifBitmapLoaderTest`**. Local `./gradlew` still requires JDK **8–15**.
