...
- **Gallery file:** Camera shots and **gallery picks** are saved upright (EXIF applied via `ReceiptExifBitmapLoader`) with `MediaStore` under **`Pictures/Mountain Money`** (`MediaStoreReceiptSaver`). `ReceiptPickerUriNormalizer` copies any non–Mountain Money URI before OCR so gallery and camera share stable app-owned URIs.
