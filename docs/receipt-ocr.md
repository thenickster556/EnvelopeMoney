...
- **Gallery file:** Camera shots and **gallery picks** are saved upright (EXIF applied via `ReceiptExifBitmapLoader`) with `MediaStore` under **`Pictures/Mountain Money`** (`MediaStoreReceiptSaver`). `ReceiptPickerUriNormalizer` imports any non–Mountain Money URI on the **main thread** before OCR; `ReceiptSourceDeleter` removes the picker source when the platform allows (move semantics). Gallery and camera share stable app-owned URIs for preview and `receiptImageUri`.
