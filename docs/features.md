...
Opening a receipt automatically checks its saved reference and searches `Pictures/Mountain Money` when needed, including pictures saved yesterday or earlier today. Being able to open a still-readable (app-owned) picture is not the same as library access: Android 10+ can show the app’s own photos without `READ_MEDIA_IMAGES` / `READ_EXTERNAL_STORAGE`, while yesterday and older album files stay hidden until that permission is granted. Launch and resume request photo access when any stored receipts exist; allowing it retries automatically and repairs stored history in the background. A verified repair survives restart. Recovery does not require picking or replacing the picture. If the app cannot uniquely identify a readable file, it keeps the transaction and reference and explains the outcome with an automatic-search retry action.
