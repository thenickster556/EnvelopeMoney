...
- **Receipt capture:** Add and **edit** transaction dialogs include **Camera scan** (CameraX) and **From gallery**. **Preview receipt** and **Remove receipt** (outlined buttons) appear once an image URI is attached; **Preview** opens a **fullscreen** viewer (pinch-zoom, pan, double-tap refit, 90° **view** rotation until saved). **Save rotation** (enabled when net rotation is not a multiple of 360°) confirms **Replace**, then overwrites the JPEG at the same URI with **Matrix**-rotated pixels (quality 92), reloads the image, and resets orientation; **back/close** while dirty asks to keep editing or discard. On-device OCR extracts amount, optional date, and a **best-guess** merchant—skipping obvious junk lines, **title-casing** ALL CAPS names, and favoring **bottom-up** labeled totals for restaurant/receipt, else the **largest** money candidate; **pond is always chosen manually**. Camera saves under **Pictures/Mountain Money**. Rows with `receiptImageUri` show a **photo** icon to the same preview. See [receipt-ocr.md](receipt-ocr.md).
