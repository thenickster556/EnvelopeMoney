...
Tapping Preview or a list photo tries the **stored** URI first (one-shot byte decode, no re-import), then launches fullscreen preview with `EXTRA_IMAGE_URI` and, for `content://`, Intent data plus a read grant. If that pointer is unreadable, unique `MountainMoney_*.jpg` identity or a unique same-day unused album file can repair `receiptImageUri` / `receiptImageFileName` and then open. Two unused files on the same local day stay unresolved. Gallery pick uses the document picker with a persistable read grant. Background recovery uses the same one-pass matcher after launch/resume photo access. A verified repair survives restart. Recovery does not require picking or replacing the picture. If the app cannot uniquely identify a readable file, it keeps the transaction and reference and explains the outcome with an automatic-search retry action.
