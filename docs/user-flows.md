...
4. The attached-receipt preview has a `Choose different` button: it re-opens the picker with the currently attached file excluded (empty pool toasts "No other pictures match this receipt."), so an accidental pick can be corrected at any time through the same check flow.
