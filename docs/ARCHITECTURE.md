...
- Startup schedules repair, then requests photo-library access after resume when stored receipts exist and permission is missing. Grants (including returning from Settings) re-run repair. Preview retries use the same resolver. Preview, OCR and rotation share resolution, and rotation writes the verified image URI. Preview decode and rotation run on a lifecycle-owned worker.
